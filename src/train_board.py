...
class TrainBoard:
    ...
    def __init__(self, get_new_data):
        ...
        self.heading_label = Label(config['font'], max_glyphs=len(
            config['heading_text']), anchor_point=(0, 0))
...
class Train:
    def __init__(self, parent_group, index):
        ...
        self.line_rect = Rect(
            0, y, config['train_line_width'], config['train_line_height'], fill=config['loading_line_color'])

        self.destination_label = Label(
            config['font'], max_glyphs=config['destination_max_characters'], anchor_point=(0, 0))
        ...
        self.min_label = Label(
            config['font'], max_glyphs=config['min_label_characters'], anchor_point=(0, 0))
    ...
    def set_arrival_time(self, minutes: str):
        ...
        minutes = ' ' * \
            (config['min_label_characters'] - minutes_len) + minutes
